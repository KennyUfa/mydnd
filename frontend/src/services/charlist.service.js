...
class ChampionApi {
    ...
    async UpdateAbilityHideOriginal(ability, champion_id) {
        const response = await api.patch(
    ...
    async UpdateAbilityHideCustom(ability, champion_id) {
        const response = await api.patch(
            "/dnd/characters/" + champion_id + "/custom-ability/hide-custom/",
            ability
        );
        if (response.data) {
            return response.data;
        } else {
            console.log('UpdateAbilityHideCustom', response);
        }
    }
}

...
urlpatterns = [
    ...
    path('<int:pk>/archetype-change/', ArchetypeChangeView.as_view(), name='archetype-change'),
    # Изменение отображения способности класса
    path('<int:pk>/custom-ability/hide-original/', CharacterHideOriginalAbilityView.as_view(), name='hide-original-ability'),
    # Изменение отображения кастом способности персонажа
    path('<int:pk>/custom-ability/hide-custom/', CharacterHideCustomAbilityView.as_view(), name='hide-custom-ability'),
    path('custom-ability/update/<int:pk>/', CharacterCustomAbilityUpdateView.as_view(), name='custom-ability-update'),
]

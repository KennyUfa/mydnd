...
def combine_names(apps, schema_editor):
    ...
    school = apps.get_model('dnd', 'MagicSchool')
    ...
    with open('dnd/migrations/spells.csv', 'r', newline='', encoding="UTF-8") as csvfile:
        ...
        for row in reader:
            ...
            if row[2] == "Заговор" or row[2] == "заговор":
                ...
                level = int(row[2][0])
            try:
                school_choice,_ = school.objects.get_or_create(name=row[3].split()[0])
...
def armordb(apps, schema_editor):
    ...
    with open('dnd/migrations/armore.json', 'r', encoding='utf-8') as \
        ...
        for w in weapons_json:
            ...
            db_ItemRarity.objects.get_or_create(description='Обычный')

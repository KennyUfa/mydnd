<template>
  <Accordion type="single" class="w-full" collapsible:True default-value="One">
    <!-- Блок для расы -->
    <AccordionItem value="One"
                   class="border border-black-400 rounded-2xl">
      <AccordionTrigger>Информация о расе: {{ race.name }}</AccordionTrigger>
      <AccordionContent>
        <CardHeader>
    ...
    <AccordionItem value="subrace-info"
      ...
      <AccordionTrigger>Информация о подрасе: {{
      ...
      <AccordionContent v-if="sub_race">
        <CardHeader>
          ...
          <CardDescription>{{ sub_race.description }}</CardDescription>
        </CardHeader>
          <CardHeader v-for="feature in sub_race.features" :key="feature.id" class="p-2">
            <CardTitle>{{ feature.name }}</CardTitle>
            <CardDescription>{{ feature.description }}</CardDescription>
          </CardHeader>
      </AccordionContent>

    ...
    <SheetContent class="overflow-y-auto w-full sm:max-w-3xl">
      ...
      <SheetFooter>
        <div class="flex flex-col space-y-4">
          <!-- Перебираем уровни -->
          <div v-for="(spells, level) in spells" :key="level" class="space-y-2">
            <!-- Заголовок уровня -->
            <SheetTitle>Уровень {{ level }}</SheetTitle>

            <!-- Список заклинаний -->
            <div class="flex flex-col space-y-1">
              <SheetDescription v-for="spell in spells" :key="spell.id">
                <Badge>{{ spell.name }}</Badge>
                <Button @click="addSpell(spell)" class="add-button">+
                </Button>
              </SheetDescription>
            </div>
          </div>

<template>
    <div class="spell-slots-container">
        <div v-for="levelSlots in spell_slots.slot_levels" :key="levelSlots.id"
             class="spell-level-section">
            <!--     Кнопки добавления ячеек-->
            <div class="header">
                <div class="buttons-left">
                    <button @click="addSlot(levelSlots)" class="add-button">+
                    </button>
                    <button @click="removeSlot(levelSlots)"
                            :disabled="levelSlots.spell_slot_level.count === 0"
                            class="remove-button">-
                    </button>
                </div>
                <span class="level-title"
                      v-if="levelSlots.spell_slot_level.level===0">Фокусы</span>
                <span class="level-title" v-else>Ячейки {{
                        levelSlots.spell_slot_level.level
                    }}го уровня</span>
            </div>
                ...
                >
                    <Dialog>
                        <DialogTrigger class="spell-block" as-child>
                            <Button variant="outline " class="text-balance"
                        ...
                        <DialogContent class="overflow-y-auto max-h-[80vh]">
                            <DialogHeader v-if="spell_details">
                                <DialogTitle>{{ spell_details.name }} - Уровень:
                                    {{
                                        spell_details.level
                                    }}
                                </DialogTitle>
                    ...
                    <button @click="removeSpell(levelSlots,spellIndex,
           spell.id)"
                            :disabled="levelSlots.spells.length === 0"
                            class="remove-button">Х
                    </button>

                </div>
                <change-spell
                        :level_slots="levelSlots.spell_slot_level.level"></change-spell>
            </div>
        </div>
    </div>
</template>
...
function toggleSlotUsage(levelSlots, index) {
    ...
    if (index < currentUsed) {
        ...
        levelSlots.spell_slot_level.used = Math.min(levelSlots.spell_slot_level.count, currentUsed + 1);
    }
    patchSlots(levelSlots.spell_slot_level)
}

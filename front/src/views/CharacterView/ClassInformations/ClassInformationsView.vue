<template>
  <Accordion type="single" class="w-full" collapsible>
    ...
    <AccordionItem value="class"
      ...
      <AccordionTrigger class="ml-4 font-bold uppercase text-xl italic">{{
          champion_class.name
        }}
      </AccordionTrigger>
      <AccordionContent>
        <div class="px-6">{{ champion_class.description }}</div>
        <!-- Описание способностей класса-->
        <div v-if="store.ClassAbilities.length > 0" class="">
          <h3 class="font-bold text-center uppercase py-3">
            Описание способностей</h3>
          <div
            v-for="ability in store.ClassAbilities"
            :key="ability.name"
          ...
          >
            <div class="border-2 border-red-600 rounded-lg p-4 mb-3 relative">
              ...
              <div class="pb-2">{{ ability.level }}
                уровень - {{ ability.source }}
              </div>
              <!-- Оригинальное описание -->
              <div v-if="!ability.custom_description?.hide_original"
                   v-html="renderMarkdown(ability.description)">
              </div>
    ...
    <DropdownMenu>
      <DropdownMenuTrigger
        ...
        class="items-center my-2 text-slate-900">
    ...
    <!--        архетип-->
    <AccordionItem value="arch" class="border border-black-400 rounded-2xl">
      <AccordionTrigger
        class="ml-4 font-bold uppercase text-xl italic">
        <!--                выбор архетипа-->
      ...
      <AccordionContent>
        ...
        <div v-if="store.ArchAbilities.length > 0" class="">
          <h3 class="font-bold text-center uppercase py-3">
            Описание способностей</h3>
          <div
            v-for="ability in store.ArchAbilities"
            :key="ability.name"
            :id="`ability-${ability.name.toLowerCase().replace(/\s+/g, '-')}`"
          >
    ...
    allLevelsWithAbilities" :key="index"
          ...
          <AccordionItem value=index class="card">
            <AccordionTrigger><strong>Уровень:</strong>
              {{ levelData.level }}
            </AccordionTrigger>
            <AccordionContent>
              <div class="card-row">

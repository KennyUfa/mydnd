  ...
  actions: {
    async fetchCharacter(characterId) {
      ...
      try {
        ...
        // Пробрасываем ошибку дальше для обработки в роутере
        throw error;
      } finally {
    ...
    async changeOrigin(data) {
      try {
        const response = await api.patch(
          'dnd/character/' + this.character_id + '/origin/',
          data
        );
    ...
    async changeWorldOutlook(data) {
      try {
        const response = await api.patch(
          'dnd/character/' + this.character_id + '/world-outlook/',
          data
        );
    ...
    async patchMainInfo() {
      ...
      try {
        ...
        const spellbook = useSpellBook();
        const response = await api.patch(
          'dnd/character/' + this.character_id + '/',
          data
        );
    ...
    async postSkills() {
      ...
      try {
        const response = await api.patch(
          'character/' + this.character_id + '/skills/',
          data
        );
    ...
    async switchAbilityState(data) {
      this.character.skill_state[data] =
        (this.character.skill_state[data] % 3) + 1;
    },
    async updateCharacterHit(max_hit, temp_hit) {
      try {
        const response = await api.patch(
          'dnd/character/' + this.character_id + '/max_hit/',
          {
            max_hit: max_hit,
            temp_hit: temp_hit,
          }
    ...
    async switchProtectState(data) {
      this.character.protect_state[data] =
        this.character.protect_state[data] === 1 ? 2 : 1;
    },
    async patchAbilityState() {
      ...
      try {
        const response = await api.patch(
          'character/' + this.character_id + '/skill_state/',
          data
        );
    ...
    async patchProtectSkills() {
      ...
      try {
        const response = await api.patch(
          'character/' + this.character_id + '/protect_state/',
          data
        );
    ...
    async patchPossessionBonus() {
      try {
        await api.patch(
          'dnd/character/' + this.character_id + '/possession_bonus/',
          { possession_bonus: this.character.possession_bonus }
        );
      } catch (error) {
    ...
    async patchInspirationFrame() {
      try {
        await api.patch(
          'dnd/character/' + this.character_id + '/inspiration_frame/',
    ...
    async patchProtectionClass() {
      try {
        await api.patch(
          'dnd/character/' + this.character_id + '/protection_class/',
          { protection_class: this.character.protection_class }
        );
      } catch (error) {
    ...
    async patchSpeed() {
      try {
        await api.patch('dnd/character/' + this.character_id + '/speed/', {
    ...
    async heal(heal) {
      try {
        const response = await api.patch(
          'dnd/character/' + this.character_id + '/heal/',
          { heal: heal }
        );
    ...
    async damage(damage) {
      try {
        const response = await api.patch(
          'dnd/character/' + this.character_id + '/damage/',
          { damage: damage }
        );
        this.character.current_hit = response.data.current_hit;
